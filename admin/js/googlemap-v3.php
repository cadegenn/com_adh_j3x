...
$GOOGLEMAP_API_KEY=$params->get("googlemap_apikey");

/**
 * @TODO
 * . supprimer le addOverlay qui ne sert plus à rien
 *  
 */

$GOOGLE_DEFAULT_GLAT="46.71109";
$GOOGLE_DEFAULT_GLNG="1.7191036";
$GOOGLEMAP_DEFAULT_ZOOM=6;
$GOOGLEMAP_DEFAULT_TYPE="TERRAIN";

?>

<script src="http://maps.googleapis.com/maps/api/js?key=<?php echo $GOOGLEMAP_API_KEY; ?>&sensor=false&callback=initialize"></script>
...
/**
 * @brief	initialize()	Fonction d'initialisation 
 */
function initialize() {
	<?php // init defaults
 ...
 *
 * @param	address		adresse du marker
 ...
 */
function createMarkerFromAddress(address, categorie, titre, desc, href, draggable) {
	...
	console.log(funcName+"(): address = " + address);

	// first, reverse-geocode the address
	if (address != "") {
		...
		geocoder.geocode({'address': address}, function(results, status) {
			if (status == google.maps.GeocoderStatus.OK) {
				...
				createMarkerFromLatLng(results[0].geometry.location.lat(), results[0].geometry.location.lng(), categorie, titre, desc, href, draggable);
			} else {
				console.log(funcName+"(): Geocoder failed due to: " + status);
			}
		});
	} else {
		console.log(funcName+"(): address est NULL = " + address);
	}
 ...
 * @param	addOverlay	vrai si on veut afficher le marker tout de suite, faux si on utilise un markerManager
 */
function createMarkerFromLatLng(glat, glng, categorie, titre, desc, href, draggable) {
	var funcName = arguments.callee.toString();
	funcName = funcName.substr('function '.length);				// trim off "function "
	funcName = funcName.substr(0, funcName.indexOf('('));		// trim off everything after the function name
	...
	//My infoWindow's id = infoWindow and my map object is named map 
				...
				if (results[1]) {
					...
					while (!found) {
						...
						if (results[1].address_components[i].types[0] == "country") {
							...
							countryCode = results[1].address_components[i].short_name;
							found = true;
						}
						i++;
					...
					document.getElementById('jform_countrycode').value = countryCode;
					document.getElementById('jspan_countrycode').innerHTML = countryCode;
					//map.setZoom(11);
					/*marker = new google.maps.Marker({
						position: latlng,
						map: map
...
/** @brief update map with values in DOM controls
 * 
 */
