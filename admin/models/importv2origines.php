...
/**
 * adhModelImportV2Origines List Model
 */
class adhModelImportV2Origines extends JModelList
{
    /*
	...
	public function import($cid) {
		...
		foreach ($cid as $key => $id) {
			...
			$query_adh->select('origines.*')->from($this->database.'.origines')->where('origines.id = '.$id);
			...
			foreach( $rows as $row ) {
				...
				$origine->creation_date = null;
				$origine->modified_by = $user->id;
				...
				$query->delete('#__adh_origines')->where('id = '.$origine->id);
				$db->setQuery($query);
				$db->execute();
				// on insert le nouvel objet (qui en fait contient les anciennes data de la V2 du site)
				$db->insertObject('#__adh_origines', $origine );
			}
		}
