...
/**
 * adhModelImportV2Tarifs List Model
 */
class adhModelImportV2Tarifs extends JModelList
{
    /*
    ...
    protected function getListQuery()
    {
		...
		$query->from($this->database.'.tarifs');
	...
	public function import($cid) {
		...
		foreach ($cid as $key => $id) {
			...
			$query_adh->select('tarifs.*')->from($this->database.'.tarifs')->where('tarifs.id = '.$id);
			...
			foreach( $rows as $row ) {
				...
				$tarif = clone $row; //new stdClass();//$row;
				// on ajoute les nouveaux champs
				$tarif->published = 0;//($row->obsolete ? 2 : $categorie->published);
				$tarif->creation_date = null;
				$tarif->modified_by = $user->id;
				// on formatte les données correctement
				$tarif->published = ($row->enable ? 1 : $tarif->published);
				// on supprime les attributs de trop
				unset($tarif->enable);
				//echo("<pre>".var_dump($categorie)."</pre>");
				// on supprime les id en conflit
				$query->delete('#__adh_tarifs')->where('id = '.$tarif->id);
				$db->setQuery($query);
				$db->execute();
				// on insert le nouvel objet (qui en fait contient les anciennes data de la V2 du site)
				$db->insertObject('#__adh_tarifs', $tarif );
			}
		}
